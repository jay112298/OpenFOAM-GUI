    ...
    <div className="grid grid-cols-2 gap-6 max-w-4xl">
      <Card>
        <Field label="NACA designation" help="4- or 5-digit code: 0012, 4412, 23012.">
          <Input
            value={field("geometry.parameters.designation") ?? ""}
        ...
        <Field label="Chord" unit="m">
          <Input
            type="number"
            value={field("geometry.parameters.chord") ?? 1}
...
/* ---------------- Mesh ---------------- */
export function Mesh({ caseId, field, setField, persist }) {
  ...
  const calc = useMutation({
    mutationFn: () =>
      api.yplus(
        field("physics.reference.velocity"),
        field("geometry.parameters.chord"),
        field("physics.fluid.name") || "air",
        field("mesh.parameters.target_yplus")
      ),
    onSuccess: setYp,
  });
  const gen = useMutation({
    mutationFn: async (id) => { await persist(); return api.generate(id); },
  ...
  return (
    <div className="grid grid-cols-2 gap-6 max-w-4xl">
      <Card>
        <div className="text-sm font-semibold mb-3">Domain & refinement</div>
        {num("mesh.parameters.upstream", "Upstream extent", "chords")}
        {num("mesh.parameters.downstream", "Downstream extent", "chords")}
        {num("mesh.parameters.vertical", "Vertical extent", "chords")}
        {num("mesh.parameters.surface_refine", "Surface refinement level")}
        {num("mesh.parameters.n_layers", "Boundary layers")}
      ...
      <div>
        <Card className="mb-4">
          ...
          {yp && (
            <div className="grid grid-cols-2 gap-2 mt-3">
              ...
              <Stat label="Cf" value={yp.cf.toFixed(5)} />
              <Stat label="u_tau" value={`${yp.u_tau.toFixed(3)} m/s`} />
        ...
        <Card>
          ...
          <p className="text-xs text-[var(--muted-foreground)] mb-3">
            Writes the OpenFOAM dicts (blockMesh, snappyHexMesh, fields) from the spec.
          </p>
          <GenerateButton gen={gen} caseId={caseId} />
          {derived && (
            <div className="grid grid-cols-2 gap-2 mt-3">
...
function GenerateButton({ gen, caseId }) {
  return (
    <>
      <Button onClick={() => gen.mutate(caseId)} disabled={gen.isPending}>
        {gen.isPending ? "Generating…" : "Generate OpenFOAM case"}
      </Button>
...
/* ---------------- Physics & BCs ---------------- */
export function Physics({ field, setField }) {
  return (
    <div className="max-w-2xl">
      <Card>
        ...
        <Field label="Turbulence model">
          <Select
            options={["kOmegaSST", "kOmegaSSTLM", "kEpsilon", "realizableKE"]}
        ...
        <p className="text-xs text-[var(--muted-foreground)]">
          Boundary conditions are generated automatically: <code>freestream</code> on the far field,
          <code> noSlip</code> + wall functions on the airfoil. Turbulence inlet values (k, omega) are
...
/* ---------------- Validate ---------------- */
export function Validate({ caseId, persist }) {
  ...
  const override = useMutation({
    mutationFn: ({ rule_id, message }) => api.override(caseId, rule_id, message),
  });
              ...
              <Card key={f.rule_id} className="p-3">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    {f.suggestion && (
                      <div className="text-xs text-[var(--muted-foreground)] mt-1">→ {f.suggestion}</div>
                    )}
                  </div>
                  {f.severity === "warn" && (
...
/* ---------------- Run ---------------- */
export function Run({ caseId, persist }) {
  ...
  function openSocket(runId) {
    setLogs([]); setResiduals([]); setFields([]); setError(null); iter.current = 0;
    const ws = api.runSocket(runId);
    wsRef.current = ws;
    ws.onmessage = (ev) => {
      const msg = JSON.parse(ev.data);
      if (msg.log) setLogs((l) => [...l.slice(-400), msg.log]);
      if (msg.error) setError(msg.error);
      if (msg.residual) {
        const { field: fld, initial } = msg.residual;
  ...
  return (
    <div className="max-w-4xl">
      ...
      {logs.length > 0 && (
        <Card className="mt-4">
          <div className="text-sm font-semibold mb-2">Solver log</div>
          <div className="bg-black/50 rounded-lg p-3 h-56 overflow-auto font-mono text-xs text-green-400">
    ...
    <div className="max-w-4xl">
      ...
      {data && !data.latest && (
        <p className="text-sm text-[var(--muted-foreground)] mt-4">
          No force data yet — run the solver first.
        </p>
      )}
